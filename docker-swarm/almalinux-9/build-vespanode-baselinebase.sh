...
then
    ...
    DEVIMAGE=vespa-debug-dev-almalinux-9
    if docker build --progress=plain -t $DEVIMAGE ../../debug-dev/almalinux-9
    then
    ...
    DEVIMAGE=vespaengine/vespa-dev-almalinux-9
fi

CONTAINER_NAME=$USER-build-vespanode-baselinebase-almalinux-9
BASELINEBASE_NAME=$USER-vespanode-baselinebase-almalinux-9
